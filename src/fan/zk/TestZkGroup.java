...
public class TestZkGroup implements Watcher {
	...
	public void connect(String hosts) throws IOException, InterruptedException {
		...
		zk = new ZooKeeper(hosts, SESSION_TIMEOUT, wh); // 最后一个参数用this会调用自身的监听
														// wh 代表？
		// connectedSignal.await(); // 主线程挂起
	}

	// 加入组(可以理解成一个创建本次连接的一个组)
	public void join(String groupname, String meberName)
			...
			zk.getChildren(path, true, new AsyncCallback.Children2Callback() {
				...
				public void processResult(int rc, String path, Object ctx,
					...
					for (int i = 0; i < children.size() - 1; i++) {
						System.out.println("mempath = " + children.get(i)
								+ stat);
					}
				}
			}, null);
	...
	public Stat isexist(String groupname) throws InterruptedException,
			KeeperException {
	...
	public void write(String path, String value) throws Exception {
		...
		if (stat == null) {
			zk.create(path, value.getBytes("UTF-8"), Ids.OPEN_ACL_UNSAFE,
					CreateMode.PERSISTENT);
		} else {
